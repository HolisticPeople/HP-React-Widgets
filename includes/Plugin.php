...
class Plugin
{
    ...
    private static function setupFunnelCptHooks(): void
    {
        ...
        // Restore slug from previous value if it was auto-generated (v2.43.35)
        // This runs AFTER WordPress saves, to fix any slug reversion
        add_action('acf/save_post', [self::class, 'restoreFunnelSlugAfterSave'], 5);
    ...
    public static function preserveFunnelSlug(array $data, array $postarr): array
    {
        ...
        // Get the stored previous slug (set before save in onFunnelSave)
        $previousSlug = get_post_meta($postId, '_hp_funnel_previous_slug', true);
        $currentSlug = get_post_field('post_name', $postId);
        $submittedSlug = $data['post_name'] ?? '';
        
        // If we have a previous slug stored and the submitted slug looks auto-generated,
        // preserve the previous slug
        if (!empty($previousSlug) && !empty($currentSlug)) {
            // Check if submitted is auto-generated from title
            $autoGeneratedSlug = sanitize_title($data['post_title']);
            if (empty($submittedSlug) || $submittedSlug === $autoGeneratedSlug) {
    ...
    /**
     * Restore funnel slug after save if it was incorrectly changed.
     * 
     * This runs on acf/save_post with priority 5 (before other hooks).
     * It checks if the slug was auto-generated and restores the previous value.
     *
     * @param int $postId Post ID
     */
    public static function restoreFunnelSlugAfterSave(int $postId): void
    {
        ...
        // Get the previous slug we stored
        $previousSlug = get_post_meta($postId, '_hp_funnel_previous_slug', true);
        if (empty($previousSlug)) {
            return;
        }

        // Get what was just saved
        $currentSlug = get_post_field('post_name', $postId);
        $postTitle = get_the_title($postId);
        $autoGeneratedSlug = sanitize_title($postTitle);

        // If the current slug matches an auto-generated version but we had a different
        // previous slug, restore it
        if ($currentSlug === $autoGeneratedSlug && $previousSlug !== $autoGeneratedSlug) {
            // The slug was auto-generated - restore the previous one
            global $wpdb;
            $wpdb->update(
                $wpdb->posts,
                ['post_name' => $previousSlug],
                ['ID' => $postId],
                ['%s'],
                ['%d']
            );
            ...
            error_log("[HP-RW] Restored funnel slug from '$currentSlug' to '$previousSlug' for post $postId");
        }
    }
