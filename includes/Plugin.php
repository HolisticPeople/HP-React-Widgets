...
class Plugin
{
    ...
    private static function setupFunnelCptHooks(): void
    {
        ...
        // Capture the original slug BEFORE save begins (priority 1 = very early)
        add_filter('wp_insert_post_data', [self::class, 'captureOriginalSlug'], 1, 2);
        
        // Preserve permalink slug during save (fixes Classic Editor + ACF issue)
        add_filter('wp_insert_post_data', [self::class, 'preserveFunnelSlug'], 99, 2);
     ...
     */
    /**
     * Transient storage for original slug during save.
     */
    private static array $originalSlugs = [];

    /**
     * Capture the original slug at the START of the save process.
     * This runs with priority 1, before any other filters can modify the data.
     */
    public static function captureOriginalSlug(array $data, array $postarr): array
    {
        ...
        $postId = $postarr['ID'] ?? 0;
        if ($postId) {
            // Capture the current database slug BEFORE it's modified
            self::$originalSlugs[$postId] = get_post_field('post_name', $postId);
        }
    ...
    /**
     * Preserve funnel slug during save.
     * This runs with priority 99, after WordPress has processed the slug.
     * It restores the original slug if it was auto-generated.
     */
    public static function preserveFunnelSlug(array $data, array $postarr): array
    {
        ...
        // Get the original slug we captured at the start
        $originalSlug = self::$originalSlugs[$postId] ?? '';
        if (empty($originalSlug)) {
            return $data;
        }

        // What WordPress is about to save
        $newSlug = $data['post_name'] ?? '';
        
        // What an auto-generated slug would look like
        $autoGeneratedSlug = sanitize_title($data['post_title']);
        
        // If the new slug matches auto-generated AND differs from original,
        // it means WordPress auto-generated it - restore the original
        if ($newSlug === $autoGeneratedSlug && $originalSlug !== $autoGeneratedSlug) {
            $data['post_name'] = $originalSlug;
        }
